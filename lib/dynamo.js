...
export default class DynamoDb{
  ...
  getAllData(next){
    this.log.info({op: 'dynamodb-read'}, 'In getAllData function')
    const requestStart = Date.now();
    const params = {
      TableName: this.tableName,

    };
    // Call DynamoDB to return all data
    this.documentClient.scan(params, (err, data) => {
      if (err) {
        this.log.warn('Unable to query dynamo db', err)
      } else { 
        this.log.info({op: 'dynamodb-read', latency: Date.now() - requestStart}, 'Successfully retrieved all data from dynamo');
        next(data)
      }
    });
  }
